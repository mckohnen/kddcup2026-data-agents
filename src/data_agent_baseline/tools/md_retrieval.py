...
def retrieve_relevant_chunks(
    ...
    *,
    ...
    Otherwise BM25 ranking selects the top_k most relevant chunks, then they
    are returned in their original document order for coherence.
    """
    ...
    for i in top_indices:
        ...
        if len(chunk) > remaining:
            # Always include at least a portion of the best chunk so the caller
            # never receives an empty string from a non-empty document.
            if not selected:
